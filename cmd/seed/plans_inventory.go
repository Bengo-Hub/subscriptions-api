...
func seedInventoryPlans(ctx context.Context, tx *ent.Tx) error {
	...
	plans := []planDef{
		...
		},
		{
			...
			price:        5500.0,
			tierOrder:    1,
			tierLimits:   map[string]any{"max_warehouses": 1, "max_products": 500, "max_suppliers": 10, "max_users": 3, "inventory_max_images_per_item": 1},
			features:     []string{"stock_tracking", "low_stock_alerts", "purchase_orders", "basic_reports"},
		},
		{
			...
			price:        16500.0,
			tierOrder:    2,
			tierLimits:   map[string]any{"max_warehouses": 5, "max_products": 5000, "max_suppliers": 50, "max_users": 10, "inventory_max_images_per_item": 5},
			features:     []string{"stock_tracking", "low_stock_alerts", "purchase_orders", "basic_reports", "multi_warehouse", "batch_expiry_tracking", "supplier_portal", "advanced_analytics", "inventory_multiple_images"},
		},
		{
