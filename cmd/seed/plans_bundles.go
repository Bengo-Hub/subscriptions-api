...
// powerSuiteFeatures is the single source of truth for the PowerSuite feature set at
// tier N (1=Starter, 2=Growth, 3=Professional). Shared by the recurring MONTHLY
// PowerSuite plans and the ONE_TIME perpetual PowerSuite licenses so every tier is a
// proper superset of the one below and a license grants exactly its recurring-tier
// equivalent (no drift). NOTE: the *_YEARLY plans below are intentionally NOT rewired
// to this — their historical feature lists differ slightly (e.g. yearly Professional
// omits staff_fund_from_salary) and those rows are being retired, so they stay verbatim.
	...
	plans := []planDef{
		...
		// ── PowerSuite Bundle — Annual ───────────────────────────────────────
		{
			...
			tierLimits: map[string]any{
				...
				"overage_transactions_price_per_100_month": 200.0, "overage_sms_price_per_100": 150.0,
				"max_devices": 1, "max_cashiers": 2, "max_tables": 20,
				"inventory_max_sku": 500, "inventory_max_warehouses": 1, "inventory_max_images_per_item": 1, "max_suppliers": 10,
		...
		},
		{
			...
			tierLimits: map[string]any{
				...
				"overage_transactions_price_per_100_month": 200.0, "overage_sms_price_per_100": 150.0,
				"max_devices": 5, "max_cashiers": 10, "max_tables": 50,
				"inventory_max_sku": 5000, "inventory_max_warehouses": 5, "inventory_max_images_per_item": 5, "max_suppliers": 50,
		...
		},
		{
			...
			tierLimits: map[string]any{
				...
				"overage_transactions_price_per_100_month": 200.0, "overage_sms_price_per_100": 150.0,
				"max_devices": -1, "max_cashiers": -1, "max_tables": -1,
				"inventory_max_sku": -1, "inventory_max_warehouses": -1, "inventory_max_images_per_item": -1, "max_suppliers": -1,
