...
func seedPOSProductLinePlans(ctx context.Context, tx *ent.Tx) error {
	...
	plans := []planDef{
		// ─── Codevertex POS (Hospitality: hotels, restaurants, bars) ───────────
		{
			...
			name: "Codevertex POS Starter (Hospitality)", description: "Cafes, small restaurants & bars. POS, M-Pesa Till, shift management, standard receipts, basic inventory, table management & loyalty.",
			billingCycle: "MONTHLY", price: 2500, setupFee: 10000, tierOrder: 1,
			tierLimits: map[string]any{"max_devices": 2, "max_cashiers": 3, "max_tables": 20},
			features:   with("table_management", "loyalty_program"),
		},
		{
			id: uuid.NewSHA1(uuid.NameSpaceOID, []byte("pos:hosp:PRO")), planCode: "POS_HOSP_PRO",
			name: "Codevertex POS Professional (Hospitality)", description: "Full restaurants & bars with rooms. + Floor plans, multi-station KDS, bill splitting, recipe/BOM, eTIMS, Treasury invoicing.",
			billingCycle: "MONTHLY", price: 5500, setupFee: 18000, tierOrder: 2,
		...
		},
		{
			...
			name: "Codevertex POS Hospitality (Hotels & F&B)", description: "Full hotels with F&B, conference halls & spas. + Reception & check-in/out, room inventory, charge-to-room, guest folio, conference module, multi-outlet reports, API.",
			billingCycle: "MONTHLY", price: 9500, setupFee: 30000, tierOrder: 3,
		...
		// ─── Codevertex Duka (General retail: shops, supermarkets, hardware) ────
		{
			...
			name: "Codevertex Duka Starter (Retail)", description: "Small shops, kiosks & market stalls. POS, M-Pesa Till, basic inventory, shift management, standard receipts, barcode & loyalty.",
			billingCycle: "MONTHLY", price: 2500, setupFee: 10000, tierOrder: 1,
			tierLimits: map[string]any{"max_devices": 1, "max_cashiers": 2},
			features:   with("barcode_scanning", "loyalty_program"),
		},
		{
			id: uuid.NewSHA1(uuid.NameSpaceOID, []byte("pos:duka:PRO")), planCode: "POS_DUKA_PRO",
			name: "Codevertex Duka Professional (Retail)", description: "Supermarkets, hardware stores, boutiques & wholesalers. + Real-time stock, low-stock alerts, supplier lists, sales analytics, eTIMS.",
			billingCycle: "MONTHLY", price: 5500, setupFee: 18000, tierOrder: 2,
		...
		},
		{
			...
			name: "Codevertex Duka Enterprise (Retail Chains)", description: "Multi-branch retail chains. + Multi-branch management, consolidated reporting, advanced analytics, API access, priority support.",
			billingCycle: "MONTHLY", price: 9500, setupFee: 30000, tierOrder: 3,
		...
		// ─── Codevertex Dawa (Pharmacy: chemists, pharmacies, dispensaries) ─────
		{
			...
			name: "Codevertex Dawa Starter (Pharmacy)", description: "Small chemists & agrovets. POS, M-Pesa Till, basic inventory, shift management, receipts, eTIMS, batch/expiry & prescriptions.",
			billingCycle: "MONTHLY", price: 3500, setupFee: 12000, tierOrder: 1,
			tierLimits: map[string]any{"max_devices": 1, "max_cashiers": 2},
		...
		},
		{
			...
			name: "Codevertex Dawa Professional (Pharmacy)", description: "Full pharmacies & hospital dispensaries. + Patient history, prescription attachment, PPB audit reports, insurance claims (NHIF/private).",
			billingCycle: "MONTHLY", price: 6500, setupFee: 22000, tierOrder: 2,
			tierLimits: map[string]any{"max_devices": 5, "max_cashiers": 10},
		...
		},
		{
			...
			name: "Codevertex Dawa Enterprise (Pharmacy Chains)", description: "Pharmacy chains & wholesale distributors. + Multi-branch management, consolidated reporting, advanced analytics, API access, priority support, label printing.",
			billingCycle: "MONTHLY", price: 11000, setupFee: 35000, tierOrder: 3,
			tierLimits: map[string]any{"max_devices": -1, "max_cashiers": -1},
