...
// hospClinicalCore — patient/clinical essentials present at every Afya tier.
...
// afyaTierFeatures — full cross-service union for Afya tier N (1=Clinic, 2=Facility, 3=Hospital).
...
type afyaTier struct {
	code     string // AFYA_CLINIC / AFYA_FACILITY / AFYA_HOSPITAL
	label    string
	tier     int
	monthly  float64
	setupFee float64
...
var afyaTiers = []afyaTier{
	{code: "AFYA_CLINIC", label: "Afya Clinic", tier: 1, monthly: 7500, setupFee: 15000, oneTime: 140000, support: 18000},
	{code: "AFYA_FACILITY", label: "Afya Facility", tier: 2, monthly: 18000, setupFee: 35000, oneTime: 320000, support: 42000},
	{code: "AFYA_HOSPITAL", label: "Afya Hospital", tier: 3, monthly: 40000, setupFee: 75000, oneTime: 0, support: 90000},
}
	...
	for _, t := range afyaTiers {
		...
		if existing != nil {
			_, err = tx.SubscriptionPlan.UpdateOneID(id).
				...
				SetTierLimitsJSON(limits).SetServiceTag("hospital").SetUpdatedAt(now).Save(ctx)
		} else {
			_, err = tx.SubscriptionPlan.Create().
				...
				SetIsActive(true).SetIsPublic(true).
				SetTierOrder(t.tier).SetFreeTrialDays(14).
				SetTierLimitsJSON(limits).SetServiceTag("hospital").
