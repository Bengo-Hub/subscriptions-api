...
type UserRoleAssignment struct {
	...
	// User identifier (subscriptions_user)
	...
	subscriptions_role_user_assignments *uuid.UUID
...
type UserRoleAssignmentEdges struct {
	...
	User *SubscriptionsUser `json:"user,omitempty"`
	// Role holds the value of the role edge.
	Role *SubscriptionsRole `json:"role,omitempty"`
...
func (e UserRoleAssignmentEdges) UserOrErr() (*SubscriptionsUser, error) {
	if e.User != nil {
		return e.User, nil
	} else if e.loadedTypes[0] {
		return nil, &NotFoundError{label: subscriptionsuser.Label}
	}
...
// was not loaded in eager-loading, or loaded but was not found.
func (e UserRoleAssignmentEdges) RoleOrErr() (*SubscriptionsRole, error) {
	if e.Role != nil {
		return e.Role, nil
	} else if e.loadedTypes[1] {
		return nil, &NotFoundError{label: subscriptionsrole.Label}
	}
...
func (*UserRoleAssignment) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case userroleassignment.ForeignKeys[0]: // subscriptions_role_user_assignments
	...
	for i := range columns {
		...
		case userroleassignment.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullScanner); !ok {
				return fmt.Errorf("unexpected type %T for field subscriptions_role_user_assignments", values[i])
			} else if value.Valid {
				_m.subscriptions_role_user_assignments = new(uuid.UUID)
				*_m.subscriptions_role_user_assignments = *value.S.(*uuid.UUID)
			}
		default:
...
func (_m *UserRoleAssignment) QueryUser() *SubscriptionsUserQuery {
...
func (_m *UserRoleAssignment) QueryRole() *SubscriptionsRoleQuery {
