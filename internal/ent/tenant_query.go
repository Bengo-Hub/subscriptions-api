...
func (tq *TenantQuery) QuerySubscriptions() *TenantSubscriptionQuery {
	query := (&TenantSubscriptionClient{config: tq.config}).Query()
...
func (tq *TenantQuery) WithSubscriptions(opts ...func(*TenantSubscriptionQuery)) *TenantQuery {
	query := (&TenantSubscriptionClient{config: tq.config}).Query()
...
func (tq *TenantQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tenant, error) {
	...
	if query := tq.withSubscriptions; query != nil {
		if err := tq.loadSubscriptions(ctx, query, nodes,
			func(n *Tenant) { n.Edges.Subscriptions = []*TenantSubscription{} },
			func(n *Tenant, e *TenantSubscription) { n.Edges.Subscriptions = append(n.Edges.Subscriptions, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TenantQuery) loadSubscriptions(ctx context.Context, query *TenantSubscriptionQuery, nodes []*Tenant, init func(*Tenant), assign func(*Tenant, *TenantSubscription)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(tenantsubscription.FieldTenantID)
	}
