...
func (pq *ProductQuery) QueryProductSubscriptions() *ProductSubscriptionQuery {
	query := (&ProductSubscriptionClient{config: pq.config}).Query()
...
func (pq *ProductQuery) Clone() *ProductQuery {
	...
	return &ProductQuery{
		...
		withProductSubscriptions: pq.withProductSubscriptions.Clone(),
...
func (pq *ProductQuery) WithProductSubscriptions(opts ...func(*ProductSubscriptionQuery)) *ProductQuery {
	query := (&ProductSubscriptionClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withProductSubscriptions = query
	return pq
}
...
func (pq *ProductQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Product, error) {
	var (
		...
		_spec       = pq.querySpec()
		loadedTypes = [1]bool{
			pq.withProductSubscriptions != nil,
		}
	)
	...
	if query := pq.withProductSubscriptions; query != nil {
		if err := pq.loadProductSubscriptions(ctx, query, nodes,
			func(n *Product) { n.Edges.ProductSubscriptions = []*ProductSubscription{} },
			func(n *Product, e *ProductSubscription) {
...
func (pq *ProductQuery) loadProductSubscriptions(ctx context.Context, query *ProductSubscriptionQuery, nodes []*Product, init func(*Product), assign func(*Product, *ProductSubscription)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(productsubscription.FieldProductID)
	}
	query.Where(predicate.ProductSubscription(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(product.ProductSubscriptionsColumn), fks...))
	}))
