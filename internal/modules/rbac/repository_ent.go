...
// CreateUser persists a new subscriptions user reference.
func (r *EntRepository) CreateUser(ctx context.Context, tenantID uuid.UUID, user *SubscriptionsUser) error {
	if user == nil {
		return errors.New("user cannot be nil")
	}

	builder := r.client.SubscriptionsUser.Create().
		SetID(user.ID).
...
func (r *EntRepository) GetUser(ctx context.Context, tenantID uuid.UUID, userID uuid.UUID) (*SubscriptionsUser, error) {
	entUser, err := r.client.SubscriptionsUser.Query().
		Where(
			subscriptionsuser.ID(userID),
			subscriptionsuser.TenantID(tenantID),
		).
		Only(ctx)
	if err != nil {
...
func (r *EntRepository) GetUserByAuthServiceID(ctx context.Context, tenantID uuid.UUID, authServiceUserID uuid.UUID) (*SubscriptionsUser, error) {
	entUser, err := r.client.SubscriptionsUser.Query().
		Where(
			subscriptionsuser.AuthServiceUserID(authServiceUserID),
			subscriptionsuser.TenantID(tenantID),
		).
		Only(ctx)
	if err != nil {
...
func (r *EntRepository) UpdateUser(ctx context.Context, tenantID uuid.UUID, userID uuid.UUID, updates *UserUpdates) error {
	builder := r.client.SubscriptionsUser.Update().
		Where(
			subscriptionsuser.ID(userID),
			subscriptionsuser.TenantID(tenantID),
		)
...
func (r *EntRepository) CreateRole(ctx context.Context, tenantID uuid.UUID, role *SubscriptionsRole) error {
	if role == nil {
		return errors.New("role cannot be nil")
	}

	builder := r.client.SubscriptionsRole.Create().
		SetID(role.ID).
		SetTenantID(tenantID).
...
func (r *EntRepository) GetRole(ctx context.Context, tenantID uuid.UUID, roleID uuid.UUID) (*SubscriptionsRole, error) {
	entRole, err := r.client.SubscriptionsRole.Query().
		Where(
			subscriptionsrole.ID(roleID),
			subscriptionsrole.Or(
				subscriptionsrole.TenantID(tenantID),
				subscriptionsrole.TenantIDIsNil(),
			),
		).
		Only(ctx)
	if err != nil {
...
func (r *EntRepository) GetRoleByCode(ctx context.Context, tenantID uuid.UUID, roleCode string) (*SubscriptionsRole, error) {
	entRoles, err := r.client.SubscriptionsRole.Query().
		Where(
			subscriptionsrole.RoleCode(roleCode),
			subscriptionsrole.Or(
				subscriptionsrole.TenantID(tenantID),
				subscriptionsrole.TenantIDIsNil(),
			),
...
func (r *EntRepository) ListRoles(ctx context.Context, tenantID uuid.UUID) ([]*SubscriptionsRole, error) {
	entRoles, err := r.client.SubscriptionsRole.Query().
		Where(subscriptionsrole.Or(
			subscriptionsrole.TenantID(tenantID),
			subscriptionsrole.TenantIDIsNil(),
		)).
		All(ctx)
	...
	byCode := make(map[string]*ent.SubscriptionsRole, len(entRoles))
	...
	roles := make([]*SubscriptionsRole, 0, len(byCode))
...
func preferTenantRole(roles []*ent.SubscriptionsRole) *ent.SubscriptionsRole {
	chosen := roles[0]
...
func (r *EntRepository) CreatePermission(ctx context.Context, permission *SubscriptionsPermission) error {
	if permission == nil {
		return errors.New("permission cannot be nil")
	}

	builder := r.client.SubscriptionsPermission.Create().
		SetID(permission.ID).
...
func (r *EntRepository) GetPermission(ctx context.Context, permissionID uuid.UUID) (*SubscriptionsPermission, error) {
	entPerm, err := r.client.SubscriptionsPermission.Get(ctx, permissionID)
	if err != nil {
		if ent.IsNotFound(err) {
...
func (r *EntRepository) GetPermissionByCode(ctx context.Context, permissionCode string) (*SubscriptionsPermission, error) {
	entPerm, err := r.client.SubscriptionsPermission.Query().
		Where(subscriptionspermission.PermissionCode(permissionCode)).
		Only(ctx)
	if err != nil {
		if ent.IsNotFound(err) {
			return nil, fmt.Errorf("permission not found: %w", err)
...
func (r *EntRepository) ListPermissions(ctx context.Context, filters PermissionFilters) ([]*SubscriptionsPermission, error) {
	query := r.client.SubscriptionsPermission.Query()

	if filters.Module != nil {
		query = query.Where(subscriptionspermission.Module(*filters.Module))
	}
	if filters.Action != nil {
		query = query.Where(subscriptionspermission.Action(*filters.Action))
	}
	...
	permissions := make([]*SubscriptionsPermission, len(entPerms))
...
func (r *EntRepository) GetRolePermissions(ctx context.Context, roleID uuid.UUID) ([]*SubscriptionsPermission, error) {
	entPerms, err := r.client.SubscriptionsRole.Query().
		Where(subscriptionsrole.ID(roleID)).
		QueryPermissions().
		All(ctx)
	if err != nil {
...
func (r *EntRepository) GetUserRoles(ctx context.Context, tenantID uuid.UUID, userID uuid.UUID) ([]*SubscriptionsRole, error) {
	...
	roles := make([]*SubscriptionsRole, len(entRoles))
...
func (r *EntRepository) GetUserPermissions(ctx context.Context, tenantID uuid.UUID, userID uuid.UUID) ([]*SubscriptionsPermission, error) {
	...
	permissionMap := make(map[uuid.UUID]*SubscriptionsPermission)
	...
	permissions := make([]*SubscriptionsPermission, 0, len(permissionMap))
...
func mapEntUser(entUser *ent.SubscriptionsUser) *SubscriptionsUser {
...
func mapEntRole(entRole *ent.SubscriptionsRole) *SubscriptionsRole {
...
func mapEntPermission(entPerm *ent.SubscriptionsPermission) *SubscriptionsPermission {
	perm := &SubscriptionsPermission{
		ID:             entPerm.ID,
